  ...
  return {
    ...
    server = {
      ...
      on_attach = function(_, bufnr)
        vim.keymap.set("v", "<leader>j", function() vim.cmd.RustLsp('joinLines') end,
          { buffer = bufnr, desc = "rust: join lines", })
        vim.keymap.set("n", "<leader>cc", function() vim.cmd.RustLsp("codeAction") end,
          { buffer = bufnr, desc = "rust: code actions", })
        vim.keymap.set("n", "<leader>rT", function() vim.cmd.RustLsp({ "testables", bang = true }) end,
          { buffer = bufnr, desc = "rust: run last ran test", })
        vim.keymap.set("n", "<leader>rd", function() vim.cmd.RustLsp("openDocs") end,
          { buffer = bufnr, desc = "rust: open rustdoc for symbol under cursor" })
        vim.keymap.set("n", "<leader>rh", function() vim.cmd.RustLsp({ "view", "hir" }) end,
          { buffer = bufnr, desc = "rust: view HIR", })
        vim.keymap.set("n", "<leader>rm", function() vim.cmd.RustLsp({ "view", "mir" }) end,
          { buffer = bufnr, desc = "rust: view MIR", })
        vim.keymap.set("n", "<leader>rr", function() vim.cmd.RustLsp({ "flyCheck", "run" }) end,
